...
import run.ice.zero.base.repository.FileInfoRepository;
...
public class FileService {
    ...
    public FileData info(FileParam param) {
        ...
        if (!code.equals(x)) {
            throw new AppException(BaseError.FILE_CODE_ERROR, param.toJson());
        }
        ...
        if (null != json && !json.isEmpty()) {
            ...
            if (optional.isEmpty()) {
                throw new AppException(BaseError.FILE_NOT_EXIST, id);
            }
    ...
    private FileInfo store(File originFile) {
        ...
        try {
            ...
            log.error(e.getMessage(), e);
            throw new AppException(BaseError.FILE_READ_WRITE_ERROR, e.getMessage());
        }

        if (!file.exists() || !file.isFile() || !(file.length() > 0) || !file.canRead()) {
            log.error("复制文件失败！");
            throw new AppException(BaseError.FILE_READ_WRITE_ERROR, file.toString());
        }
