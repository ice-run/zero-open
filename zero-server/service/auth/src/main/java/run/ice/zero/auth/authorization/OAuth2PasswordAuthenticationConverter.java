...
public class OAuth2PasswordAuthenticationConverter implements AuthenticationConverter {
    ...
    public Authentication convert(HttpServletRequest request) {
        ...
        if (!OAuth2Constant.PASSWORD.equals(grantType)) {
            return null;
        }
        ...
        // username (REQUIRED)
        String username = parameters.getFirst(OAuth2Constant.USERNAME);
        if (!StringUtils.hasText(username) || parameters.get(OAuth2Constant.USERNAME).size() != 1) {
            OAuth2EndpointUtils.throwError(OAuth2ErrorCodes.INVALID_REQUEST, OAuth2Constant.USERNAME, OAuth2EndpointUtils.ACCESS_TOKEN_REQUEST_ERROR_URI);
        }

        // password (REQUIRED)
        String password = parameters.getFirst(OAuth2Constant.PASSWORD);
        if (!StringUtils.hasText(password) || parameters.get(OAuth2Constant.PASSWORD).size() != 1) {
            OAuth2EndpointUtils.throwError(OAuth2ErrorCodes.INVALID_REQUEST, OAuth2Constant.PASSWORD, OAuth2EndpointUtils.ACCESS_TOKEN_REQUEST_ERROR_URI);
        }
