...
public interface RbacApi {
    ...
    @PostExchange(url = "role-perm")
    Response<RolePermData> rolePerm(@RequestHeader(HttpHeaders.AUTHORIZATION) String authorization, @RequestBody @Valid Request<No> request);
    ...
    @PostExchange(url = "role-perm-list")
    Response<List<PermData>> rolePermList(@RequestBody @Valid Request<IdParam> request);
    ...
    @PostExchange(url = "role-perm-upsert")
    Response<Ok> rolePermUpsert(@RequestBody @Valid Request<RolePermUpsert> request);
